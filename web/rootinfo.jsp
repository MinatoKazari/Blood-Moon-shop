...
<%
    ...
    rs = sql.executeQuery("SELECT * FROM rootinformation where rootID ='"+session.getAttribute("RootID")+"'");
    try{
%>
<html>
<head>
...
<div id="userinfo">
    <dl>
        <dt><a>
                <% while (rs.next()) { %>
        <dd>id：<%out.print(rs.getString(1));%><br></dd>
        <dd>帐号：<%out.print(rs.getString(2));%><br></dd>
        ...
        <dd>创建帐号时间：<%out.print(rs.getString(9));%><br></dd>
        <dd>最后一次更新时间：<%out.print(rs.getString(10));%><br></dd>
        <% } %>
        <%
            rs.close();
            sql.close();
